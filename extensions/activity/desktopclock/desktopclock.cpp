...
public:
  ...
  cherry_kit::Window *mMainWindow;
  cherry_kit::Widget *mWindowContentWidget;

  cherry_kit::Widget *mLayoutWidget;
  QGraphicsLinearLayout *m_main_layout;

  cherry_kit::ToolBar *m_tool_bar;
  cherry_kit::Label *m_timezone_label;
  cherry_kit::ClockWidget *m_clock_widget;

  cherry_kit::ItemView *m_timezone_list;
...
DesktopClockActivity::DesktopClockActivity(QGraphicsObject *object)
    : cherry_kit::DesktopActivity(object), d(new PrivateDesktopClock) {}
...
void DesktopClockActivity::createFrameWindow(const QRectF &window_geometry,
  ...
  d->mMainWindow = new cherry_kit::Window();
...
void DesktopClockActivity::create_window(const QRectF &window_geometry,
  ...
  d->mWindowContentWidget = new cherry_kit::Widget(d->mMainWindow);
  d->mWindowContentWidget->setGeometry(window_geometry);

  d->mLayoutWidget = new cherry_kit::Widget(d->mWindowContentWidget);
  ...
  /* add clock widget */
  d->m_clock_widget = new cherry_kit::ClockWidget(d->mLayoutWidget);
  ...
  /* Add Toolbar Widget */
  d->m_tool_bar = new cherry_kit::ToolBar(d->mLayoutWidget);

  d->m_timezone_label = new cherry_kit::Label(d->m_tool_bar);
  ...
  d->m_timezone_list = new cherry_kit::ItemView(d->mLayoutWidget);
  d->m_timezone_list->set_view_geometry(_timezone_table_rect);

  d->m_timezone_list->on_item_removed([](cherry_kit::ModelViewItem *a_item) {
    if (a_item)
      delete a_item;
  });
...
void DesktopClockActivity::PrivateDesktopClock::add_time_zone(
  ...
  cherry_kit::ModelViewItem *_item_ptr = new cherry_kit::ModelViewItem();
  cherry_kit::Label *_item_label_ptr = new cherry_kit::Label(m_timezone_list);
  ...
  _item_ptr->on_activated([&](cherry_kit::ModelViewItem *a_item) {
    if (!a_item)
      return;
  ...
  _item_ptr->on_view_removed([](cherry_kit::ModelViewItem *a_item) {
...
cherry_kit::Window *DesktopClockActivity::window() const {
...
void DesktopClockActivity::onToolBarAction(const QString &str) {
  if (str.toLower() == "timezone") {
    ...
    if (viewport()) {
      ...
      if (_space) {
        cherry_kit::DesktopActivityPtr _timezone = viewport()->create_activity(
