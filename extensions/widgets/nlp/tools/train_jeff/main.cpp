...
qtz_string_list qtz_build_word_list_from_string(const std::string &a_str) {
...
qtz_build_sentance(const qtz_string_list &a_sentence_vlist,
  ...
  std::for_each(std::begin(a_entity_vlist), std::end(a_entity_vlist),
                [&](qtz_entity_type a_entity) {
...
std::vector<ner_training_instance> qtz_build_data_set() {
  ...
  for (tinyxml2::XMLNode *child = root->FirstChild(); child != NULL;
    ...
    for (tinyxml2::XMLElement *sentence = child->FirstChildElement("text");
         sentence != NULL; sentence = sentence->NextSiblingElement("text")) {
    ...
    for (tinyxml2::XMLElement *tag = child->FirstChildElement("tag");
         tag != NULL; tag = tag->NextSiblingElement("tag")) {
...
int main(int argc, char **argv) {
  ...
  if (argc != 2) {
    cout << "You must give the path to the MITIE English "
            "total_word_feature_extractor.dat file." << endl;
    cout << "So run this program with a command like: " << endl;
    cout << "./train_ner_example "
  ...
  std::for_each(
      std::begin(sentence_list), std::end(sentence_list),
      [&](ner_training_instance a_sentence) { trainer.add(a_sentence); });
  ...
  /*
    ...
    ";
        // chunks[i] defines a half open range in sentence3 that contains the
    entity.
