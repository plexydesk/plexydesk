...
QGraphicsItem *VideoPlugin::item() {
  ...
  if (videoEngine) {
    connect(videoEngine, SIGNAL(data(QVariant &)), this,
            SLOT(data(QVariant &)));
    connect(this, SIGNAL(sendData(QVariant &)), videoEngine,
            SLOT(pushData(QVariant &)));
  } else {
    qDebug() << "DataSource Was Null"
             << "VideoPlugin::VideoPlugin(QObject * object)" << endl;
  }
