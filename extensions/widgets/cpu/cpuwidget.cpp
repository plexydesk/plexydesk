                             ...
                             QWidget *)
{
  ...
  p->drawPixmap(QRect(-(ceil(_meter_hand.width() / 2)),
  ...
  p->translate(_cpu_bg.width() / 2, _cpu_bg.width() / 2);
  p->drawPixmap(QRect(-(thedot.width() / 2), -(thedot.height() / 2),
                      thedot.width(), thedot.height()),
                QPixmap(thedot));
  p->restore();
  ...
  p->drawText(QRect(70, 108, 64, 64), Qt::AlignCenter,
                                 ...
                                 QWidget *)
{
  /*p->setRenderHints(QPainter::SmoothPixmapTransform |QPainter::Antialiasing
     ...
     p->drawText(QRect(8,5,64,64), Qt::AlignCenter
     ,"Cpu\n"+QString("%1").arg(percen)+"%" );*/
}
