...
void VPlayer::setFileName(const QString &name)
{
  ...
  if (file->exists()) {
    ...
    if (av_open_input_file(&d->pFormatCtx, name.toLatin1(), NULL, 0, NULL) !=
        0) {
      qDebug() << "av_open_input_file"
               << "Failed" << endl;
    }

    if (av_find_stream_info(d->pFormatCtx) < 0) {
      qDebug() << "av_find_stream_info"
               << "Failed" << endl;
    }
    ...
    d->numBytes = avpicture_get_size(PIX_FMT_RGBA32, d->pCodecCtx->width,
                                     d->pCodecCtx->height);

    qDebug() << "Byte Allocatoin = " << d->numBytes << endl;
    d->buffer = (uint8_t *)av_malloc(d->numBytes * sizeof(uint8_t));

    avpicture_fill((AVPicture *)d->pFrameRGB, d->buffer, PIX_FMT_RGBA32,
                   d->pCodecCtx->width, d->pCodecCtx->height);
    ...
    /*
        while (av_read_frame(d->pFormatCtx, &d->packet)>=0) {
                  ...
                  if (d->packet.stream_index==d->videoStream) {
                            ...
                            avcodec_decode_video(d->pCodecCtx, d->pFrame,
       &d->frameFinished,d->packet.data, d->packet.size);

                 if (d->frameFinished) {

                img_convert((AVPicture *)d->pFrameRGB, PIX_FMT_RGBA32,
       (AVPicture*)d->pFrame,PIX_FMT_YUV420P,d->pCodecCtx->width,d->pCodecCtx->height);

                QImage
       img(d->pFrameRGB->data[0],d->pCodecCtx->width,d->pCodecCtx->height,QImage::Format_ARGB32);
