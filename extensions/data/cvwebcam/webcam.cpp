...
QRect WebCamData::detectFace(const char *faceData) {
  ...
  if (!d->mCascade) {
    qDebug() << Q_FUNC_INFO << ": "
             << "Error incorrect Haar classifier cascade";
    return QRect();
  }
  ...
  if (d->mFaceSeq && d->mFaceSeq->total) {
    ...
    // qDebug() << "Radius : " << radius << " X: " << center.x << "Y : " <<
    // center.y;
    // histogram
    ...
    cvConvertScale(d->histogram->bins, d->histogram->bins,
                   max ? 255.0 / max : 0, 0);
...
QVariantMap WebCamData::readAll() { return d->dataMap; }
