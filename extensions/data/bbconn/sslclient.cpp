...
void SSLClient::connectedToServer() { qDebug() << Q_FUNC_INFO; }
  ...
  Q_FOREACH(QSslError error, errors) {
    if (error.error() == QSslError::SelfSignedCertificate) {
      QMessageBox::StandardButton result = QMessageBox::question(
                                             0, "SSL Error", QString("PlexyDesk is trying to connect to %1 with a "
                                                 "Self Signed Certificate, Approve ?")
...
void SSLClient::receiveMessage() { qDebug() << Q_FUNC_INFO; }

void SSLClient::connectionClosed() { qDebug() << Q_FUNC_INFO; }
