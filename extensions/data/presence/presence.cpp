 ...
 * Class destructor
 */
PresenceData::~PresenceData() { delete d; }
...
void PresenceData::init() {
   ...
   */
  connect(d->m_accountManager, SIGNAL(accountCreated(const QString &)), this,
          SLOT(accountCreated(const QString &)));
  connect(d->m_accountManager,
          SIGNAL(accountValidityChanged(const QString &, bool)), this,
          SLOT(accountValidityChanged(const QString &, bool)));
  connect(d->m_accountManager, SIGNAL(accountRemoved(const QString &)), this,
...
void PresenceData::onAccountReady(
   ...
   */
  foreach(const QString & path, pathList) { d->createAccountDataSource(path); }
}
