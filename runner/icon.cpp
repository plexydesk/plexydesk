...
Icon::Icon(IconProviderPtr icon, QPlexyMime *mime, const QRectF &rect,
           QWidget * /*embeddedWidget*/)
    : AbstractDesktopWidget(rect), d(new Private) {
  d->valid = false;
  d->iconprovider = icon;
  d->id = 0;
  d->mime = mime;
...
void Icon::loadContent() { d->mime->fromFileName(d->path); }
...
void Icon::drawText(QPainter *painter, const QRectF & /*rect*/) {
  ...
  QString label =
      fm.elidedText(d->text, Qt::ElideRight, boundingRect().width());
  QRectF tr = QRectF(0.0, y, boundingRect().width(),
                     boundingRect().height() - (d->icon.height()));
