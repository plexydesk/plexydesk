...
IconJob::IconJob(QObject *parent) : PendingJob(parent), d(new Private) {
  ...
  d->iconpaths << QDir::homePath() + QLatin1String("/.icons/") +
                      Config::getInstance()->getInstance()->iconTheme() + slash;

  QStringList xdg = QString(qgetenv("XDG_DATA_DIRS")).split(':');
  foreach(QString path, xdg) {
    d->iconpaths << path + QLatin1String("/icons/") +
  ...
  d->iconpaths << QLatin1String("/usr/share/pixmaps/")
               ...
               << Config::getInstance()->plexydeskBasePath() +
                      QLatin1String("/share/plexy/skins") + slash +
                      Config::getInstance()->getInstance()->iconTheme() +
                      slash + QLatin1String("icons") + slash;
  connect(this, SIGNAL(newJob()), this, SLOT(handleJob()),
          Qt::DirectConnection);
}

IconJob::~IconJob() { delete d; }

QPixmap IconJob::iconPixmap() const { return d->pixmap; }
