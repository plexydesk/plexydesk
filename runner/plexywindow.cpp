...
PlexyWindows::PlexyWindows(Display *dsp, Window win, XWindowAttributes *attr,
...
void PlexyWindows::bind() {
  ...
  if (!d->pixmap) {
    ...
    setRect(QRectF(d->attrib.x, d->attrib.y, d->attrib.width / 5,
                   d->attrib.height / 5));
...
void PlexyWindows::Configured(bool isNotify, int x, int y, int width,
                              int height, int border, PlexyWindows *aboveWin,
...
void PlexyWindows::Damaged(XRectangle *rect) {
  bind();
  if (!rect) {
    XRectangle allrect = { d->attrib.x, d->attrib.y, d->attrib.width,
                           d->attrib.height };
    Damaged(&allrect);
    return;
  }
...
void PlexyWindows::Resized(int x, int y, int width, int height, int border) {
  ...
  qDebug() << d->attrib.width << d->attrib.height << d->attrib.border_width
           << endl;

  if (width != d->attrib.width || height != d->attrib.height ||
      border != d->attrib.border_width || d->attrib.override_redirect) {
    qDebug() << Q_FUNC_INFO << endl;
    if (d->isRedirected) {
      qDebug() << Q_FUNC_INFO << endl;
      ReleaseWindow();
