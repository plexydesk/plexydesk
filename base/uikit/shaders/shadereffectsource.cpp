...
/*!
    ...
    \brief The ShaderEffectSource object encapsulates the source content for the
   ShaderEffectItem.
    ...
    Requirement for the ability to use of shaders is that the application is
   either using
    opengl graphicssystem or has set QGLWidget as the viewport to
   QDeclarativeView (recommended way).

    ShaderEffectSource object encapsulates the source content so that it can be
   utilized in ShaderEffectItem.
    Source content can be a live QML object tree, or a snapshot of QML object
   tree.
...
/*!
    ...
    This property holds the Item which is used as the source for the shader
   effect.
    If the item has children, those are included as well.

    \note When source item content is passed to the ShaderEffectItem(s), it is
   always clipped to the boundingrect of the
...
void ShaderEffectSource::setSourceItem(QDeclarativeItem *item) {
  ...
  if (m_sourceItem) {
    ...
    connect(m_sourceItem, SIGNAL(widthChanged()), this,
            SLOT(markSourceSizeDirty()));
    connect(m_sourceItem, SIGNAL(heightChanged()), this,
            SLOT(markSourceSizeDirty()));
  }
...
/*!
    ...
    If other value than Qt.rect(0,0,0,0) is assigned to this property, it is
   interpreted as
    specifying a relative source rectangle for the source content.

    For example, setting Qt.rect(-10.0, -10.0, 120.0, 120.0) for a source that
   has width and height
    of 100 pixels would produce 10 pixels margins to each side of the source.

    Margins are useful when the original content is wanted to be spread outside
...
/*!
    ...
    This property holds the optimization flag to define whether the source item
   content is changing or
    static.

    If value true is assigned to this property, source item content is
   re-rendered into a
    texture for every frame. Setting the value to false improves the performance
   as it skips
    rendering the source item (and its chidleren) and instead immediately passes
   the previously
...
/*!
    ...
    \brief the flag tells whether source item content is changing between
   frames.
*/
...
/*!
    ...
    This property defines the wrap parameter for the source after it has been
   mapped as a texture.

    \list
    \o WrapMode.ClampToEdge - Causes texturecoordinates to be clamped to the
   range [ 1/2*N , 1 - 1/2*N ], where N is the texture width.
    \o WrapMode.RepeatHorizontally - Causes the integer part of the horizontal
   texturecoordinate to be ignored; the GL uses only the fractional part,
   thereby creating a horizontal repeating pattern.
    \o WrapMode.RepeatVertically - Causes the integer part of the vertical
   texturecoordinate to be ignored; the GL uses only the fractional part,
   thereby creating a vertical repeating pattern.
    \o WrapMode.Repeat - Causes the integer part of both the horizontal and
   vertical texturecoordinates to be ignored; the GL uses only the fractional
...
/*!
    ...
    \brief the wrap parameter for the source after it has been mapped as a
   texture.
*/
...
/*!
    ...
    This method is useful when ShaderEffectSource::live has been set to false
   and
    the changes in the source item content is desired to be made visible for the
   shaders.
...
void ShaderEffectSource::updateBackbuffer() {
  ...
  if (size.height() > 0 && size.width() > 0) {
    ...
    if (!m_fbo) {
      ...
      if (m_fbo->size() != size ||
          m_fbo->format().internalTextureFormat() != GLenum(m_format)) {
        delete m_fbo;
        m_fbo = 0;
        m_fbo = new ShaderEffectBuffer(size, format);
      }
