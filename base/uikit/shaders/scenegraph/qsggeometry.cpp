...
/*!
    \class QSGGeometry
    \brief The QSGGeometry class provides low-level storage for graphics
   primitives
    in the QML Scene Graph.

    The QSGGeometry class provides a few convenience attributes and attribute
   accessors
    by default. The defaultAttributes_Point2D() function returns attributes to
   be used
    in normal solid color rectangles, while the
   defaultAttributes_TexturedPoint2D function
...
/*!
   ...
   accumulated
    ...
    Geometry objects are constructed with GL_TRIANGLE_STRIP as default drawing
   mode.
 ...
 */
void QSGGeometry::setDrawingMode(GLenum mode) { m_drawing_mode = mode; }
...
/*!
    Resizes the vertex and index data of this geometry object to fit \a
   vertexCount
