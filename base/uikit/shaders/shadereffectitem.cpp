...
/*!
    ...
    \brief The ShaderEffectItem object alters the output of given item with
OpenGL shaders.
    \inherits Item
    ...
    This element provides preliminary support for embedding OpenGL shader code
into QML,
    ...
    Qt OpenGL graphicssystem or is forced to use OpenGL by setting QGLWidget as
the viewport to QDeclarativeView (recommened way).

    ShaderEffectItem internal behaviour is such that during the paint event it
first renders its
    ShaderEffectSource items into a OpenGL framebuffer object which can be used
as a texture. If the ShaderEffectSource is defined to be an image,
    it is directly uploaded as a texture. The texture(s) containing the source
pixelcontent are then bound to graphics
    pipeline texture units. Finally a textured mesh is passed to the vertex- and
fragmentshaders which
    then produce the final output for the ShaderEffectItem. It is possible to
alter the mesh structure by defining
    the amount vertices it contains, but currently it is not possible to import
complex 3D-models to be used as the mesh.

    It is possible to define one or more ShaderEffectItems to be a
ShaderEffectSource for other ShaderEffectItems, but ShaderEffectItem
    should never be declared as a child element of its source item(s) because it
would cause circular loop in the painting.

    A standard set of vertex attributes are provided for the shaders:

    \list
    \o qt_Vertex - The primary position of the vertex.
    \o qt_MultiTexCoord0 - The texture co-ordinate at each vertex for texture
unit 0.
    \endlist
    ...
    \o qt_ModelViewProjectionMatrix - current 4x4 transformation matrix of the
item.
    \endlist

    Furthermore, it is possible to utilize automatic QML propertybinding into
vertex- and fragment shader
    ...
    \endtable
    \note
    The uniform precision definitions in the above table are not strict, it is
possible to choose the uniform
    precision based on what is the most suitable for the shader code for that
particular uniform.


    The below example uses fragment shader to create simple wiggly effect to a
text label.
    Automatic property binding takes care of binding the properties to the
uniforms if their
    names are identical. ShaderEffectSource referring to textLabel is bound to
sampler2D uniform inside the fragment
    shader code.
...
Rectangle {
    ...
    ShaderEffectItem {
        property variant source: ShaderEffectSource { sourceItem: textLabel;
hideSource: true }
        ...
        void main(void)
        {
            ...
            wiggledTexCoord.s += sin(4.0 * 3.141592653589 * wiggledTexCoord.t) *
wiggleAmount;
...
ShaderEffectItem::~ShaderEffectItem() { reset(); }
...
/*!
    ...
    \note
    By default the pixel data in textures is stored in 32-bit premultiplied
   alpha format.
...
/*!
    ...
    This property defines to how many triangles the item is divided into before
   its
    vertices are passed to the vertex shader.

    Triangles are defined as triangle strips and the amount of triangles can be
   controlled
  ...
  if (!m_checkedViewportUpdateMode) {
    ...
    if (s) {
      ...
      for (int i = 0; i < views.count(); i++) {
        if (views[i]->viewportUpdateMode() !=
            QGraphicsView::FullViewportUpdate) {
          qWarning() << "ShaderEffectItem::checkViewportUpdateMode - consider "
                        "setting QGraphicsView::FullViewportUpdate mode with "
                        "OpenGL!";
        }
      }
    }
...
void ShaderEffectItem::paint(QPainter *painter,
...
void ShaderEffectItem::renderEffect(QPainter *painter,
  ...
  combinedMatrix.scale(2.0 / painter->device()->width(),
                       -2.0 / painter->device()->height(), 1.0);
  combinedMatrix.translate(-painter->device()->width() / 2.0,
  ...
  if (m_geometry.indexCount())
    glDrawElements(m_geometry.drawingMode(), m_geometry.indexCount(),
                   m_geometry.indexType(), m_geometry.indexData());
  else
...
void ShaderEffectItem::updateEffectState(const QMatrix4x4 &matrix) {
  ...
  if (m_respectsOpacity)
    m_program.setUniformValue("qt_Opacity",
  ...
  for (it = m_uniformNames.begin(); it != m_uniformNames.end(); ++it) {
    ...
    switch (v.type()) {
      ...
      case QVariant::Double:
        m_program.setUniformValue(name.constData(),
                                  (float)qvariant_cast<double>(v));
        break;
      case QVariant::Transform:
        m_program.setUniformValue(name.constData(),
                                  qvariant_cast<QTransform>(v));
        break;
      case QVariant::Int:
      ...
      case QVariant::RectF: {
        QRectF r = v.toRectF();
        m_program.setUniformValue(name.constData(), r.x(), r.y(), r.width(),
                                  r.height());
      } break;
      case QVariant::Vector3D:
        m_program.setUniformValue(name.constData(),
                                  qvariant_cast<QVector3D>(v));
        break;
      default:
        break;
    }
...
static inline int size_of_type(GLenum type) {
  static int sizes[] = {
    ...
    sizeof(int),  sizeof(unsigned int),  sizeof(float), 2,
    3,            4,                     sizeof(double)
  };
...
void ShaderEffectItem::bindGeometry() {
  ...
  for (int j = 0; j < m_attributeNames.size(); ++j) {
    if (!*attrNames[j])
      continue;
    Q_ASSERT_X(j < m_geometry.attributeCount(),
    ...
    Q_ASSERT_X(j == a.position, "ShaderEffectItem::bindGeometry()",
    ...
    m_program.setAttributeArray(
        a.position, (GLfloat *)(((char *)m_geometry.vertexData()) + offset),
        a.tupleSize, m_geometry.stride());
    // glVertexAttribPointer(a.position, a.tupleSize, a.type, normalize,
    // m_geometry.stride(), (char *) m_geometry.vertexData() + offset);
...
void ShaderEffectItem::markDirty() { update(); }
...
void ShaderEffectItem::setSource(const QVariant &var, int index) {
  ...
  if (var.isNull()) {
    ...
    qWarning("Could not assign source of type '%s' to property '%s'.",
             var.typeName(), source.name.constData());
    return;
  }
  ...
  if (!source.item)
    qWarning(
        "Could not assign property '%s', did not implement QDeclarativeItem.",
        source.name.constData());

  if (!source.source)
    qWarning(
        "Could not assign property '%s', did not implement ShaderEffectSource.",
        source.name.constData());
  ...
  if (source.item && source.item->parentItem() == 0) {
    ...
    // Unlike in scenegraph, we cannot set item invisible here because
    // qgraphicsview would optimize it away.
  }

  // Unlike in scenegraph, ref counting is used to optimize memory consumption.
  // Sources themself may free fbos when not referenced.
  ...
  for (it = m_uniformNames.begin(); it != m_uniformNames.end(); ++it) {
    ...
    if (pi >= 0) {
      ...
      if (!mp.hasNotifySignal())
        qWarning("ShaderEffectItem: property '%s' does not have notification "
                 "method!",
                 it->constData());
      ...
      qWarning("ShaderEffectItem: '%s' does not have a matching property!",
               it->constData());
    }
  }
  for (int i = 0; i < m_sources.size(); ++i) {
    ...
    if (pi >= 0) {
      ...
      connect(source.mapper, SIGNAL(mapped(int)), this,
              SLOT(changeSource(int)));
    } else {
      qWarning("ShaderEffectItem: '%s' does not have a matching source!",
               source.name.constData());
    }
  }
}
...
void ShaderEffectItem::updateShaderProgram() {
  ...
  for (int i = 0; i < m_attributeNames.size(); ++i) {
    m_program.bindAttributeLocation(m_attributeNames.at(i),
...
void ShaderEffectItem::lookThroughShaderCode(const QString &code) {
  ...
  // In human readable form: attribute|uniform [lowp|mediump|highp] <type>
  // <name>
  static QRegExp re(
      QLatin1String("\\b(attribute|uniform)\\b\\s*\\b(?:lowp|mediump|highp)?"
                    "\\b\\s*\\b(\\w+)\\b\\s*\\b(\\w+)"));
  Q_ASSERT(re.isValid());
  ...
  while ((pos = re.indexIn(wideCode, pos + 1)) != -1) {
    ...
    if (decl == "attribute") {
      if (name == qt_postion_attribute_name) {
        ...
        qWarning("ShaderEffectItem: Attribute \'%s\' not recognized.",
                 name.constData());
      }
    } else {
...
void ShaderEffectItem::handleVisibilityChange() { setActive(isVisible()); }
}
