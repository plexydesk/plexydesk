...
// completely optional warning, but you can go nuts if you forget to set the
// scroll mode right
...
// returns a new point with both coordinates having the abs. value of the
// original one
...
// returns a new point with all components of p1 multiplied by the corresponding
// components of p2
...
// returns a new point with all components of p1 divided by the corresponding
// components of p2
inline QPointF operator/(const QPointF &p1, const QPointF &p2)
{
  return QPointF(p1.x() / p2.x(), p1.y() / p2.y());
...
inline int qSign(qreal r) { return (r < 0) ? -1 : ((r > 0) ? 1 : 0); }
...
static qreal differentialForProgress(const QEasingCurve &curve, qreal pos)
{
  ...
  qreal d = (curve.valueForProgress(right) - curve.valueForProgress(left)) /
            qreal(dx);

  // qScrollerDebug() << "differentialForProgress(type: " << curve.type() << ",
  // pos: " << pos << ") = " << d;
...
static qreal progressForValue(const QEasingCurve &curve, qreal value)
{
  if (curve.type() >= QEasingCurve::InElastic &&
    ...
    qWarning("progressForValue(): QEasingCurves of type %d do not have an "
             "inverse, since they are not injective.",
             curve.type());
    return value;
  }
...
{
public:
  QScrollTimer(QtScrollerPrivate *_d) : d(_d), ignoreUpdate(false), skip(0) {}
...
/*!
    \class QScroller
    \brief The QScroller class enables kinetic scrolling for any scrolling
   widget or graphics item.
    \class QtScroller
    \brief The QtScroller class enables kinetic scrolling for any scrolling
   widget or graphics item.
    \since 4.8
    ...
    The scrolled QObjects receive a QScrollPrepareEvent whenever the scroller
   needs to
    update its geometry information and a QScrollEvent whenever the content of
   the object should
    actually be scrolled.

    The scroller uses the global QAbstractAnimation timer to generate its
   QScrollEvents. This
    can be changed with QtScrollerProperties::FrameRate on a per-QtScroller
   basis.
   ...
   via
    QtScrollerProperties, we recommend that you leave them all at their default,
   platform optimized
...
/*!
    Returns \c true if a QtScroller object was already created for \a target; \c
   false otherwise.
...
/*!
    \fn QtScroller::scrollerPropertiesChanged(const QtScrollerProperties
   &newProperties);
...
/*! \property QtScroller::scrollerProperties
    ...
    The properties are used by the QtScroller to determine its scrolling
   behavior.
*/
...
void QtScroller::setScrollerProperties(const QtScrollerProperties &sp)
{
  ...
  if (d->properties != sp) {
    ...
    // we need to force the recalculation here, since the overshootPolicy may
    // have changed and
...
*/
Qt::GestureType QtScroller::grabGesture(QObject *target,
                                        ScrollerGestureType scrollGestureType)
{
  ...
  if (target->isWidgetType()) {
    ...
    if ((view = qobject_cast<QAbstractItemView *>(widget->parentWidget())) &&
      ...
      if (view->verticalScrollMode() != QAbstractItemView::ScrollPerPixel)
        qWarning("QtScroller::grabGesture() was called on a QAbstractItemView "
                 "with verticaScrollMode not set to ScrollPerPixel");
      if (view->horizontalScrollMode() != QAbstractItemView::ScrollPerPixel)
        qWarning("QtScroller::grabGesture() was called on a QAbstractItemView "
                 "with horizontalScrollMode not set to ScrollPerPixel");
    }
...
{
  Q_ASSERT(
    target); // you can't create a scroller without a target in any normal way
  Q_D(QtScroller);
  d->init();
}
...
/*!
    ...
    QtScroller emits this signal whenever the state changes. \a newState is the
   new State.
...
/*!
   ...
   QPointF.
    ...
    \note Please note that this value should be physically correct. The actual
   DPI settings
    that Qt returns for the display may be reported wrongly on purpose by the
   underlying
...
QPointF QtScroller::pixelPerMeter() const
{
  ...
  if (QGraphicsObject *go = qobject_cast<QGraphicsObject *>(d->target)) {
    QTransform viewtr;
    // TODO: the first view isn't really correct - maybe use an additional field
...
/*!
    Returns the current scrolling velocity in meter per second when the state is
...
/*!
    Starts scrolling the widget so that point \a pos is at the top-left position
   in
    the viewport.
...
/*! \overload
    ...
    This version will reach its destination position in \a scrollTime
   milliseconds.
*/
void QtScroller::scrollTo(const QPointF &pos, int scrollTime)
{
  ...
  qScrollerDebug() << "QtScroller::scrollTo(req:" << pos
                   << " [pix] / snap:" << newpos << ", " << scrollTime
                   << " [ms])";
  ...
  d->createScrollToSegments(vel.x(), time, newpos.x(), Qt::Horizontal,
                            QtScrollerPrivate::ScrollTypeScrollTo);
  d->createScrollToSegments(vel.y(), time, newpos.y(), Qt::Vertical,
                            QtScrollerPrivate::ScrollTypeScrollTo);
...
/*!
    ...
    viewport with additional margins specified in pixels by \a xmargin and \a
   ymargin around
    the rect.

    In cases where it is not possible to fit the rect plus margins inside the
   viewport the contents
   ...
   after a platform-defined
    ...
    \sa scrollTo
*/
void QtScroller::ensureVisible(const QRectF &rect, qreal xmargin,
                               qreal ymargin)
{
  // we could make this adjustable via QtScrollerProperties
                               ...
                               int scrollTime)
{
  ...
  qScrollerDebug() << "QtScroller::ensureVisible(" << rect << " [pix], "
                   << xmargin << " [pix], " << ymargin << " [pix], "
                   << scrollTime << "[ms])";
...
/*! This function resends the QScrollPrepareEvent.
    ...
    scrolling.
    Calling this function while in the Inactive state is useless as the prepare
   event
...
{
  switch (input) {
  case QtScroller::InputPress:
    return "press";
  case QtScroller::InputMove:
    return "move";
  case QtScroller::InputRelease:
    return "release";
  default:
    return "(invalid)";
  }
...
/*!
    This function is used by gesture recognizers to inform the scroller about a
   new input event.
    The scroller changes its internal state() according to the input event and
   its attached
    scroller properties. The scroller doesn't distinguish between the kind of
   input device the
    event came from. Therefore the event needs to be split into the \a input
   type, a \a position and a
    milli-second \a timestamp.  The \a position needs to be in the target's
   coordinate system.

    The return value is \c true if the event should be consumed by the calling
   filter or \c false
    if the event should be forwarded to the control.

    \note Using grabGesture() should be sufficient for most use cases.
*/
bool QtScroller::handleInput(Input input, const QPointF &position,
                             qint64 timestamp)
{
  Q_D(QtScroller);

  qScrollerDebug() << "QtScroller::handleInput(" << input << ", "
                   << d->stateName(d->state) << ", " << position << ", "
                   << timestamp << ")";
  struct statechange {
    State state;
    Input input;
    typedef bool (QtScrollerPrivate::*inputhandler_t)(const QPointF &position,
        qint64 timestamp);
  ...
  statechange statechanges[] = {
    {
      QtScroller::Inactive, InputPress,
      &QtScrollerPrivate::pressWhileInactive
    },
    { QtScroller::Pressed, InputMove, &QtScrollerPrivate::moveWhilePressed },
    {
      QtScroller::Pressed, InputRelease,
      &QtScrollerPrivate::releaseWhilePressed
    },
    { QtScroller::Dragging, InputMove, &QtScrollerPrivate::moveWhileDragging },
    {
      QtScroller::Dragging, InputRelease,
      &QtScrollerPrivate::releaseWhileDragging
    },
    {
      QtScroller::Scrolling, InputPress,
      &QtScrollerPrivate::pressWhileScrolling
    }
  };
...
QPointF QtScrollerPrivate::realDpi(int screen)
{
  ...
  if (ptrXRRSizes && ptrXRRRootToScreen) {
    ...
    Qt::HANDLE root =
      QX11Info::appRootWindow(screen == -1 ? QX11Info::appScreen() : screen);
    int randrscreen =
      (root != 0) ? ptrXRRRootToScreen(QX11Info::display(), root) : -1;

    structXRRScreenSize *sizes = ptrXRRSizes(
                                   QX11Info::display(), randrscreen == -1 ? 0 : randrscreen, &nsizes);

    if (nsizes > 0 && sizes && sizes->width && sizes->height && sizes->mwidth &&
        sizes->mheight) {
      qScrollerDebug() << "XRandR DPI:"
                       << QPointF(qreal(25.4) * qreal(sizes->width) /
                                  qreal(sizes->mwidth),
                                  qreal(25.4) * qreal(sizes->height) /
                                  qreal(sizes->mheight));
...
*/
QPointF QtScrollerPrivate::dpi() const { return pixelPerMeter * qreal(0.0254); }
...
*/
void QtScrollerPrivate::updateVelocity(const QPointF &deltaPixelRaw,
                                       qint64 deltaTime)
{
  if (deltaTime <= 0) {
    return;
  ...
  qScrollerDebug() << "QtScroller::updateVelocity(" << deltaPixelRaw
                   << " [delta pix], " << deltaTime << " [delta ms])";

  // faster than 2.5mm/ms seems bogus (that would be a screen height in ~20 ms)
  if (((deltaPixelRaw / qreal(deltaTime)).manhattanLength() /
       ((ppm.x() + ppm.y()) / 2) * 1000) > qreal(2.5))
    deltaPixel = deltaPixelRaw * qreal(2.5) * ppm / 1000 /
                 (deltaPixelRaw / qreal(deltaTime)).manhattanLength();
  ...
  qreal smoothing = sp->dragVelocitySmoothingFactor *
                    qMin(qreal(deltaTime), qreal(50)) / qreal(50);
  ...
  if ((releaseVelocity != QPointF(0, 0)) && (deltaTime < 100)) {
    qScrollerDebug() << "SMOOTHED from " << newv << " to "
                     << newv *smoothing +
                     releaseVelocity * (qreal(1) - smoothing);
    ...
    if (!newv.x() || (qSign(releaseVelocity.x()) == qSign(newv.x())))
      newv.setX(newv.x() * smoothing +
                releaseVelocity.x() * (qreal(1) - smoothing));
    if (!newv.y() || (qSign(releaseVelocity.y()) == qSign(newv.y())))
      newv.setY(newv.y() * smoothing +
                releaseVelocity.y() * (qreal(1) - smoothing));
  } else {
    qScrollerDebug() << "NO SMOOTHING to " << newv;
  }

  releaseVelocity.setX(
    qBound(-sp->maximumVelocity, newv.x(), sp->maximumVelocity));
  releaseVelocity.setY(
    qBound(-sp->maximumVelocity, newv.y(), sp->maximumVelocity));

  qScrollerDebug() << "  --> new velocity:" << releaseVelocity;
}

void QtScrollerPrivate::pushSegment(ScrollType type, qreal deltaTime,
                                    qreal stopProgress, qreal startPos,
                                    qreal deltaPos, qreal stopPos,
                                    ...
                                    Qt::Orientation orientation)
{
  ...
  if (orientation == Qt::Horizontal && !xSegments.isEmpty())
    s.startTime = xSegments.last().startTime +
                  xSegments.last().deltaTime * xSegments.last().stopProgress;
  else if (orientation == Qt::Vertical && !ySegments.isEmpty())
    s.startTime = ySegments.last().startTime +
                  ySegments.last().deltaTime * ySegments.last().stopProgress;
  else {
...
/*! \internal
    Clears the old segments and recalculates them if the current segments are
   not longer valid
*/
...
*/
qreal QtScrollerPrivate::scrollingSegmentsEndPos(
...
bool QtScrollerPrivate::scrollingSegmentsValid(Qt::Orientation orientation)
{
  ...
  if (stopPos == minPos ||
      stopPos == maxPos) { // the begin and the end of the list are always ok
    return true;
  }
...
/*! \internal
   Creates the sections needed to scroll to the specific \a endPos to the
   segments queue.
*/
void QtScrollerPrivate::createScrollToSegments(qreal v, qreal deltaTime,
    qreal endPos,
    Qt::Orientation orientation,
    ScrollType type)
{
  ...
  qScrollerDebug() << "+++ createScrollToSegments: t:" << deltaTime
                   << "ep:" << endPos << "o:" << int(orientation);

  const QtScrollerPropertiesPrivate *sp = properties.d.data();

  qreal startPos = (orientation == Qt::Horizontal)
                   ? contentPosition.x() + overshootPosition.x()
                   : contentPosition.y() + overshootPosition.y();
  qreal deltaPos = (endPos - startPos) / 2;

  pushSegment(type, deltaTime * qreal(0.3), qreal(1.0), startPos, deltaPos,
              startPos + deltaPos, QEasingCurve::InQuad, orientation);
  pushSegment(type, deltaTime * qreal(0.7), qreal(1.0), startPos + deltaPos,
              deltaPos, endPos, sp->scrollingCurve.type(), orientation);
}

/*! \internal
*/
void QtScrollerPrivate::createScrollingSegments(qreal v, qreal startPos,
    qreal ppm,
    Qt::Orientation orientation)
{
  const QtScrollerPropertiesPrivate *sp = properties.d.data();
  ...
  bool noOvershoot = (policy == QtScrollerProperties::OvershootAlwaysOff) ||
                     !sp->overshootScrollDistanceFactor;
  bool canOvershoot = !noOvershoot && (alwaysOvershoot || maxPos);

  qScrollerDebug() << "+++ createScrollingSegments: s:" << startPos
                   << "maxPos:" << maxPos << "o:" << int(orientation);
  ...
  qreal deltaPos = qSign(v) * deltaTime * deltaTime * qreal(0.5) *
                   sp->decelerationFactor * ppm;
  ...
  qScrollerDebug() << "  Real Delta:" << lowerSnapPos << "-" << nextSnap << "-"
                   << higherSnapPos;
  ...
  if (qAbs(v) < sp->minimumVelocity) {
    ...
    pushSegment(ScrollTypeFlick, sp->snapTime * qreal(0.3), qreal(1.0),
                startPos, midPos - startPos, midPos, QEasingCurve::InQuad,
                orientation);
    pushSegment(ScrollTypeFlick, sp->snapTime * qreal(0.7), qreal(1.0), midPos,
                endPos - midPos, endPos, sp->scrollingCurve.type(),
                orientation);
    return;
  }
  ...
  if (v > 0 && !qIsNaN(higherSnapPos)) {
    ...
    qreal stopProgress = progressForValue(
                           sp->scrollingCurve, qAbs((stopPos - startPos) / deltaPos));

    if (!canOvershoot) {
      qScrollerDebug() << "Overshoot stopp:" << stopProgress;

      pushSegment(ScrollTypeFlick, deltaTime, stopProgress, startPos, endPos,
                  stopPos, sp->scrollingCurve.type(), orientation);
    } else {
      qreal oDeltaTime = sp->overshootScrollTime;
      qreal oStopProgress =
        qMin(stopProgress + oDeltaTime * qreal(0.3) / deltaTime, qreal(1));
      qreal oDistance =
        startPos +
        deltaPos * sp->scrollingCurve.valueForProgress(oStopProgress) -
        stopPos;
      qreal oMaxDistance =
        qSign(oDistance) * (viewSize * sp->overshootScrollDistanceFactor);
      ...
      pushSegment(ScrollTypeOvershoot, oDeltaTime * qreal(0.7), qreal(1.0),
                  stopPos + oDistance, -oDistance, stopPos,
                  sp->scrollingCurve.type(), orientation);
    }
    return;
  }

  pushSegment(ScrollTypeFlick, deltaTime, qreal(1.0), startPos, deltaPos,
...
bool QtScrollerPrivate::prepareScrolling(const QPointF &position)
{
  ...
  qScrollerDebug() << "QScrollPrepareEvent returned from" << target << "with"
                   << spe.isAccepted() << "mcp:" << spe.contentPosRange()
                   << "cp:" << spe.contentPos();
  if (spe.isAccepted()) {
    ...
    if (QGraphicsObject *go = qobject_cast<QGraphicsObject *>(target)) {
      // TODO: the first view isn't really correct - maybe use an additional
      // field in the prepare event?
      if (go->scene() && !go->scene()->views().isEmpty()) {
...
void QtScrollerPrivate::handleDrag(const QPointF &position, qint64 timestamp)
{
  ...
  if (sp->axisLockThreshold) {
    ...
    if (dx || dy) {
      ...
      // qScrollerDebug() << "QtScroller::handleDrag() -- axis lock:" << alpha
      // << " / " << axisLockThreshold << "- isvertical:" << vertical << "- dx:"
      // << dx << "- dy:" << dy;
  ...
  //        setContentPositionHelper(q->contentPosition() - overshootDistance -
  // deltaPixel);
  //        dragDistance = QPointF(0, 0);
  //    } else {
  dragDistance += deltaPixel;
  //    }
  qDebug() << "######################" << deltaPixel << position.y()
           << lastPosition.y();
  if (canScrollX) {
...
bool QtScrollerPrivate::pressWhileInactive(const QPointF &position,
    qint64 timestamp)
{
...
bool QtScrollerPrivate::moveWhilePressed(const QPointF &position,
    qint64 timestamp)
{
  Q_Q(QtScroller);
  const QtScrollerPropertiesPrivate *sp = properties.d.data();
  ...
  bool moveStarted =
    (((deltaPixel / ppm).manhattanLength()) > sp->dragStartDistance);

  // check the direction of the mouse drag and abort if it's too much in the
  // wrong direction.
  if (moveStarted) {
    QRectF max = contentPosRange;
  ...
  if (moveAborted) {
    ...
    deltaPixel = deltaPixel - deltaPixel * (sp->dragStartDistance /
                                            deltaPixel.manhattanLength());
...
bool QtScrollerPrivate::moveWhileDragging(const QPointF &position,
    qint64 timestamp)
{
  // handleDrag updates lastPosition, lastTimestamp and velocity
...
void QtScrollerPrivate::timerEventWhileDragging()
{
  if (dragDistance != QPointF(0, 0)) {
    qScrollerDebug() << "QtScroller::timerEventWhileDragging() -- dragDistance:"
                     << dragDistance;
...
bool QtScrollerPrivate::releaseWhileDragging(const QPointF &position,
    qint64 timestamp)
{
  Q_Q(QtScroller);
  const QtScrollerPropertiesPrivate *sp = properties.d.data();
  ...
  QPointF deltaPixel = position - pressPosition;
  if (((deltaPixel / q->pixelPerMeter()).manhattanLength()) >
    ...
    if ((oldVelocity != QPointF(0, 0)) && sp->acceleratingFlickMaximumTime &&
        ((timestamp - pressTimestamp) <
         qint64(sp->acceleratingFlickMaximumTime * 1000))) {
  ...
  QPointF ppm = q->pixelPerMeter();
  createScrollingSegments(releaseVelocity.x(),
                          contentPosition.x() + overshootPosition.x(), ppm.x(),
                          Qt::Horizontal);
  createScrollingSegments(releaseVelocity.y(),
                          contentPosition.y() + overshootPosition.y(), ppm.y(),
                          Qt::Vertical);

  qScrollerDebug() << "QtScroller::releaseWhileDragging() -- velocity:"
                   << releaseVelocity
...
bool QtScrollerPrivate::pressWhileScrolling(const QPointF &position,
    qint64 timestamp)
{
  Q_Q(QtScroller);
...
void QtScrollerPrivate::setState(QtScroller::State newstate)
{
  ...
  case QtScroller::Inactive:
    ...
    // send the last scroll event (but only after the current state change was
    // finished)
    if (!firstScroll) {
      sendLastScroll = true;
    }
  ...
  if (sendLastScroll) {
    QtScrollEvent se(contentPosition, overshootPosition,
                     QtScrollEvent::ScrollFinished);
    sendEvent(target, &se);
    firstScroll = true;
  }
...
/*! \internal
    ...
    It will also indicate a new overshooting condition by the overshootX and
   oversthootY flags.
    ...
    \a deltaPos is the amount of pixels the current content position should be
   moved
*/
void QtScrollerPrivate::setContentPositionHelperDragging(
  const QPointF &deltaPos)
{
  ...
  qScrollerDebug() << "QtScroller::setContentPositionHelperDragging("
                   << deltaPos << " [pix])";
  qScrollerDebug() << "  --> overshoot:" << overshootPosition
                   << "- old pos:" << oldPos << "- new pos:" << newPos;

  QPointF oldClampedPos = clampToRect(oldPos, contentPosRange);
  QPointF newClampedPos = clampToRect(newPos, contentPosRange);

  // --- handle overshooting and stop if the coordinate is going back inside the
  // normal area
  bool alwaysOvershootX =
    (sp->hOvershootPolicy == QtScrollerProperties::OvershootAlwaysOn);
  bool alwaysOvershootY =
    (sp->vOvershootPolicy == QtScrollerProperties::OvershootAlwaysOn);
  bool noOvershootX =
    (sp->hOvershootPolicy == QtScrollerProperties::OvershootAlwaysOff) ||
    ((state == QtScroller::Dragging) && !sp->overshootDragResistanceFactor) ||
    !sp->overshootDragDistanceFactor;
  bool noOvershootY =
    (sp->vOvershootPolicy == QtScrollerProperties::OvershootAlwaysOff) ||
    ((state == QtScroller::Dragging) && !sp->overshootDragResistanceFactor) ||
    !sp->overshootDragDistanceFactor;
  bool canOvershootX =
    !noOvershootX && (alwaysOvershootX || contentPosRange.width());
  bool canOvershootY =
    !noOvershootY && (alwaysOvershootY || contentPosRange.height());
  ...
  qScrollerDebug() << "  --> noOs:" << noOvershootX
                   << "drf:" << sp->overshootDragResistanceFactor
                   << "mdf:" << sp->overshootScrollDistanceFactor
                   << "ossP:" << sp->hOvershootPolicy;
  qScrollerDebug() << "  --> canOS:" << canOvershootX
                   << "newOS:" << newOvershootX << "maxOS:" << maxOvershootX;
  ...
  QtScrollEvent se(contentPosition, overshootPosition,
                   firstScroll ? QtScrollEvent::ScrollStarted
                   : QtScrollEvent::ScrollUpdated);
  sendEvent(target, &se);
  firstScroll = false;

  qScrollerDebug() << "  --> new position:" << newClampedPos
                   << "- new overshoot:" << overshootPosition
                   << "- overshoot x/y?:" << overshootPosition;
}

qreal QtScrollerPrivate::nextSegmentPosition(QQueue<ScrollSegment> &segments,
    qint64 now, qreal oldPos)
{
  qreal pos = oldPos;
...
/*! \internal
    ...
    If \a direction >0 it will return the next snap point that is larger than
   the current position.
    If \a direction <0 it will return the next snap point that is smaller than
   the current position.
    If \a direction ==0 it will return the nearest snap point (or the current
   position if we are already
    on a snap point.
    Returns the nearest snap position or NaN if no such point could be found.
 */
qreal QtScrollerPrivate::nextSnapPos(qreal p, int dir,
                                     Qt::Orientation orientation)
{
  ...
  if (orientation == Qt::Horizontal) {
    ...
    if (snapIntervalX > 0.0) {
      ...
      if (dir > 0) {
        ...
        qreal last =
          qFloor((maxPos - first) / snapIntervalX) * snapIntervalX + first;
        if (p >= last) {
          snapPos = last;
        } else {
    ...
    if (snapIntervalY > 0.0) {
      ...
      if (dir > 0) {
        ...
        qreal last =
          qFloor((maxPos - first) / snapIntervalY) * snapIntervalY + first;
        if (p >= last) {
          snapPos = last;
        } else {
...
/*!
   ...
   but the scroll area is currently not dragged.
    \value Dragging The scroll area is currently following the touch point or
   mouse.
...
/*!
   ...
   QtScroller gesture recognizer.
    ...
    \value LeftMouseButtonGesture The gesture recognizer will only trigger on
   left mouse button events.
    \value MiddleMouseButtonGesture The gesture recognizer will only trigger on
   middle mouse button events.
    \value RightMouseButtonGesture The gesture recognizer will only trigger on
...
/*!
    ...
    This enum contains an input device agnostic view of input events that are
   relevant for QtScroller.

    \value InputPress   The user pressed the input device (e.g.
   QEvent::MouseButtonPress,
    ...
    \value InputRelease The user released the input device (e.g.
