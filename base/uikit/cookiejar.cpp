...
void CookieJar::load()
{
  ...
  QStringList list;
  list << "facebook.com";
  cookieSettings.setValue("allow", QVariant(list));
  m_exceptions_block =
...
void CookieJar::save()
{
  ...
  QSettings cookieSettings(directory + QLatin1String("/cookies.ini"),
                           QSettings::IniFormat);
                                  ...
                                  const QUrl &url)
{
  ...
  bool eBlock =
    qBinaryFind(m_exceptions_block.begin(), m_exceptions_block.end(), host) !=
    m_exceptions_block.end();
  bool eAllow =
    qBinaryFind(m_exceptions_allow.begin(), m_exceptions_allow.end(), host) !=
    m_exceptions_allow.end();
  bool eAllowSession = qBinaryFind(m_exceptions_allowForSession.begin(),
                                   m_exceptions_allowForSession.end(),
                                   host) != m_exceptions_allowForSession.end();
  ...
  : QDialog(parent)
{
  ...
  for (int i = 0; i < model->columnCount(); ++i){
      ...
      case 4:
          header =
  fm.width(QDateTime::currentDateTime().toString(Qt::LocalDate));
          break;
      }
...
CookiesExceptionsDialog::CookiesExceptionsDialog(CookieJar *cookieJar,
    QWidget *parent)
  : QDialog(parent), m_cookieJar(cookieJar)
{
  /*
  setupUi(this);
  setWindowFlags(Qt::Sheet);
  connect(removeButton, SIGNAL(clicked()), exceptionTable, SLOT(removeOne()));
  connect(removeAllButton, SIGNAL(clicked()), exceptionTable,
  SLOT(removeAll()));
  ...
  connect(allowForSessionButton, SIGNAL(clicked()), this,
  SLOT(allowForSession()));
