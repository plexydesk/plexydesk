...
/*!
    ...
    The scroll prepare event is send before scrolling (usually by QtScroller) is
   started.
    The object receiving this event should set viewportSize, maxContentPos and
   contentPos.
    It also should accept this event to indicate that scrolling should be
   started.

    It is not guaranteed that a QtScrollEvent will be send after an acceepted
    QtScrollPrepareEvent, e.g. in a case where the maximum content position is
   (0,0).
...
/*!
    ...
    The \a startPos is the position of a touch or mouse event that started the
   scrolling.
*/
...
*/
QPointF QtScrollPrepareEvent::startPos() const { return d->startPos; }
...
/*!
    ...
    \sa setViewportSize()
*/
QSizeF QtScrollPrepareEvent::viewportSize() const { return d->viewportSize; }
...
*/
QPointF QtScrollPrepareEvent::contentPos() const { return d->contentPos; }
...
/*!
   ...
   QGraphicsObject.
    ...
    Some care should be taken that no conflicting QtScrollEvents are sent from
   two
...
/*!
    ...
    \value ScrollUpdated Set for all but the first and the last scroll event of
   a scroll activity.
...
*/
QPointF QtScrollEvent::contentPos() const { return d->contentPos; }
...
/*!
    ...
    \sa QtScroller
*/
QPointF QtScrollEvent::overshootDistance() const { return d->overshoot; }

/*!
    Returns the current scroll state as a combination of ScrollStateFlag values.
    ScrollStarted (or ScrollFinished) will be set, if this scroll event is the
   first (or last) event in a scrolling activity.
    Please note that both values can be set at the same time, if the activity
   consists of a single QScrollEvent.
    All other scroll events in between will have their state set to
   ScrollUpdated.

    A widget could for example revert selections when scrolling is started and
   stopped.
    \sa isLast()
*/
