...
QtScrollerFilter::QtScrollerFilter() : ignoreMove(false) {}
...
void QtScrollerFilter::add(QObject *target) {
  ...
  if (QAbstractScrollArea *area =
          qobject_cast<QAbstractScrollArea *>(target->parent()))
    target = target->parent();
  if (QAbstractKineticScroller *m5s =
          target->property("kineticScroller")
  ...
  if (QWebView *web = qobject_cast<QWebView *>(target)) {
    if (web->page() && web->page()->mainFrame()) {
      web->page()->mainFrame()->setScrollBarPolicy(Qt::Vertical,
                                                   Qt::ScrollBarAlwaysOff);
      web->page()->mainFrame()->setScrollBarPolicy(Qt::Horizontal,
                                                   Qt::ScrollBarAlwaysOff);
    }
  }
#endif
...
void QtScrollerFilter::remove(QObject *target) {
  disconnect(QtScroller::scroller(target),
             SIGNAL(stateChanged(QtScroller::State)), this,
             SLOT(stateChanged(QtScroller::State)));
  ...
  if (QtScroller *scroller = qobject_cast<QtScroller *>(sender())) {
    if (QObject *o = scroller->target()) {
      if (o->isWidgetType()) {
        ...
        if (w->parentWidget()) {
          if (QAbstractScrollArea *area =
                  qobject_cast<QAbstractScrollArea *>(w->parentWidget())) {
            if (area->viewport() == w) {
              if (QAbstractItemView *view =
                      qobject_cast<QAbstractItemView *>(area))
...
bool QtScrollerFilter::eventFilter_QWebView(QWebView *view, QEvent *event) {
  switch (event->type()) {
    case QtScrollPrepareEvent::ScrollPrepare: {
      ...
      if (view->page()) {
        if (QWebFrame *frame =
                scrollingFrameAt_QWebView(view, se->startPos().toPoint())) {
          ...
          se->setContentPosRange(
              QRectF(0, 0, qMax(0, s.width()), qMax(0, s.height())));
    ...
    case QtScrollEvent::Scroll: {
      if (QWebFrame *frame = scrollingFrames.value(view)) {
        ...
        //    frame->d->frame->selection()->setSelection(d->oldWebSelection,
...
QWebFrame *QtScrollerFilter::scrollingFrameAt_QWebView(
    QWebView *view, const QPoint &pos) const {
  if (!view->page())
    return 0;
  ...
  if (!vsbrect.isEmpty() &&
      vsbrect.contains(hitTest.pos() - hitFrame->scrollPosition()))
    return 0;
  if (!hsbrect.isEmpty() &&
      hsbrect.contains(hitTest.pos() - hitFrame->scrollPosition()))
    return 0;
...
bool QtScrollerFilter::eventFilter_QAbstractScrollArea(
    QAbstractScrollArea *area, QEvent *event) {
  switch (event->type()) {
    case QtScrollPrepareEvent::ScrollPrepare: {
      QtScrollPrepareEvent *se = static_cast<QtScrollPrepareEvent *>(event);
      if (canStartScrollingAt_QAbstractScrollArea(area,
                                                  se->startPos().toPoint())) {
    ...
    case QtScrollEvent::Scroll: {
      ...
      if (!ptrBeginPanningFeedback)
        ptrBeginPanningFeedback = (PtrBeginPanningFeedback)QLibrary::resolve(
            QLatin1String("UxTheme"), "BeginPanningFeedback");
      if (!ptrUpdatePanningFeedback)
        ptrUpdatePanningFeedback = (PtrUpdatePanningFeedback)QLibrary::resolve(
            QLatin1String("UxTheme"), "UpdatePanningFeedback");
      if (!ptrEndPanningFeedback)
        ptrEndPanningFeedback = (PtrEndPanningFeedback)QLibrary::resolve(
            QLatin1String("UxTheme"), "EndPanningFeedback");

      if (ptrBeginPanningFeedback && ptrUpdatePanningFeedback &&
        ...
        if (!se->overshootDistance().isNull())
          ptrUpdatePanningFeedback(wid, -se->overshootDistance().x(),
                                   -se->overshootDistance().y(), false);
        if (se->overshootDistance().isNull() && !os.isNull())
...
struct HackedAbstractItemView : public QAbstractItemView {
  void hackedExecuteDelayedItemsLayout() { executeDelayedItemsLayout(); }
};

void QtScrollerFilter::stateChanged_QAbstractItemView(QAbstractItemView *view,
  ...
  switch (state) {
    ...
    case QtScroller::Dragging:
      ...
      if (view->selectionModel()) {
        view->selectionModel()->select(oldSelection,
                                       QItemSelectionModel::ClearAndSelect);
        view->selectionModel()->setCurrentIndex(oldCurrent,
...
bool QtScrollerFilter::eventFilter_QAbstractItemView(QAbstractItemView *view,
                                                     QEvent *event) {
  switch (event->type()) {
    case QtScrollPrepareEvent::ScrollPrepare:
      static_cast<HackedAbstractItemView *>(view)
          ->hackedExecuteDelayedItemsLayout();
      break;
...
bool QtScrollerFilter::canStartScrollingAt_QAbstractScrollArea(
    QAbstractScrollArea *area, const QPoint &startPos) const {
