...
static QMouseEvent *copyMouseEvent(QEvent *e) {
  switch (e->type()) {
    ...
    case QEvent::MouseMove: {
      ...
      return new QMouseEvent(me->type(), QPoint(0, 0), me->globalPos(),
...
#if 1
      QEvent::Type met = me->type() == QEvent::GraphicsSceneMousePress
                             ? QEvent::MouseButtonPress
                             : (me->type() == QEvent::GraphicsSceneMouseRelease
                                    ? QEvent::MouseButtonRelease
                                    : QEvent::MouseMove);
      return new QMouseEvent(met, QPoint(0, 0), me->screenPos(), me->button(),
...
public:
  enum { UngrabMouseBefore = 1, RegrabMouseAfterwards = 2 };
  ...
  bool shouldEventBeIgnored(QEvent *) const { return sendingEvent; }

  bool isDelaying() const { return !pressDelayEvent.isNull(); }
  ...
  bool released(QEvent *e, bool scrollerWasActive, bool scrollerIsActive) {
    ...
    if (pressDelayEvent && mouseTarget && !scrollerIsActive) {
      ...
      qFGDebug() << "QFG: re-sending mouse press (due to release) for "
                 << mouseTarget;
      sendMouseEvent(pressDelayEvent.data(), UngrabMouseBefore);

      qFGDebug() << "QFG: faking mouse release (due to release) for "
                 << mouseTarget;
      sendMouseEvent(releaseEvent.data());
      result = true; // consume this event
    } else if (mouseTarget && scrollerIsActive) {
      // we grabbed the mouse expicitly when the scroller became active, so undo
      // that now
  ...
  void scrollerWasIntercepted() {
    qFGDebug() << "QFG: deleting delayed mouse press, since scroller was only "
                  "intercepted";
    if (pressDelayEvent) {
    ...
    if (pressDelayEvent) {
      ...
      /*for (int i = 0; i < 32; ++i) {
          if (mouseButtons & (1 << i)) {
              ...
              qFGDebug() << "QFG: sending a fake mouse release at far-far-away
      to " << mouseTarget;
      ...
      qFGDebug() << "QFG: sending a fake mouse release at far-far-away to "
                 << mouseTarget;
      QMouseEvent re(QEvent::MouseButtonRelease, QPoint(), farFarAway,
                     mouseButton, QApplication::mouseButtons() & ~mouseButton,
                     QApplication::keyboardModifiers());
      sendMouseEvent(&re, RegrabMouseAfterwards);
      // don't clear the mouseTarget just yet, since we need to explicitly
      // ungrab the mouse on release!
    }
  }
  ...
  void timerEvent(QTimerEvent *e) {
    if (e->timerId() == pressDelayTimer) {
      if (pressDelayEvent && mouseTarget) {
        qFGDebug() << "QFG: timer event: re-sending mouse press to "
                   << mouseTarget;
        sendMouseEvent(pressDelayEvent.data(), UngrabMouseBefore);
      }
      pressDelayEvent.reset(0);
  ...
  void sendMouseEvent(QMouseEvent *me, int flags = 0) {
    if (mouseTarget) {
      ...
      if (mouseTarget->parentWidget()) {
        if (QGraphicsView *gv =
                qobject_cast<QGraphicsView *>(mouseTarget->parentWidget())) {
          if (gv->scene())
      ...
      if (me) {
        QMouseEvent copy(
            me->type(), mouseTarget->mapFromGlobal(me->globalPos()),
...
/*!
    ...
    \class QFlickGesture
    \since 4.8
    \brief The QFlickGesture class describes a flicking gesture made by the
   user.
    \ingroup gestures
    The QFlickGesture is more complex than the QPanGesture that uses QScroller
   and QScrollerProperties
    to decide if it is triggered.
    This gesture is reacting on touch event as compared to the
   QMouseFlickGesture.
...
/*!
    \internal
*/
QtFlickGesture::QtFlickGesture(QObject *receiver_, Qt::MouseButton button,
                               QObject *parent)
    : QGesture(parent),
      receiver(receiver_),
      receiverScroller(0),
  ...
  receiverScroller = (receiver && QtScroller::hasScroller(receiver))
                         ? QtScroller::scroller(receiver)
                         : 0;
}
...
/*! \internal
    The recognize function detects a touch event suitable to start the attached
   QScroller.
    The QFlickGesture will be triggered as soon as the scroller is no longer in
   the state
    QScroller::Inactive or QScroller::Pressed. It will be finished or canceled
    at the next QEvent::TouchEnd.
    Note that the QScroller might continue scrolling (kinetically) at this
   point.
 */
QGestureRecognizer::Result QtFlickGestureRecognizer::recognize(QGesture *state,
  ...
  QGraphicsObject *receiverGraphicsObject =
      qobject_cast<QGraphicsObject *>(d->receiver);

  // this is only set for events that we inject into the event loop via
  // sendEvent()
  ...
  // qFGDebug() << "FlickGesture
  // "<<state<<"watched:"<<watched<<"receiver"<<d->receiver<<"event"<<event->type()<<"button"<<button;
...
#if 0 // QNativeGestureEvent is in qevent_p.h, which is of no use for this
      // solution
  ...
  switch (event->type()) {
    ...
    case QEvent::MouseMove:
#ifdef Q_OS_SYMBIAN
      // Qt on Symbian tracks the button state internally, while Qt on
      // Win/Mac/Unix
    ...
    case QEvent::TouchUpdate:
      ...
      if (te->device()->type() == QTouchDevice::TouchPad) {
        ...
        point = te->touchPoints().at(0).startScenePos() +
                ((te->touchPoints().at(0).scenePos() -
                  te->touchPoints().at(0).startScenePos()) +
                 (te->touchPoints().at(1).scenePos() -
                  te->touchPoints().at(1).startScenePos())) /
                    2;
      } else {                              // TouchScreen
  ...
  if (inputType == QtScroller::InputPress) {
    foreach(QtScroller * as, QtScroller::activeScrollers()) {
      if (as != scroller) {
        ...
        } else if (QGraphicsObject *go =
                       qobject_cast<QGraphicsObject *>(as->target())) {
          if (go->scene() && !go->scene()->views().isEmpty()) {
            foreach(QGraphicsView * gv, go->scene()->views())
            scrollerRegion |=
                gv->mapFromScene(go->mapToScene(go->boundingRect()))
                    .translated(gv->mapToGlobal(QPoint(0, 0)));
          }
        }
        // active scrollers always have priority
        if (scrollerRegion.contains(globalPos)) {
          qFGDebug() << "QFG: ignoring press event at" << globalPos
                     << "since another scroller is active there";
          return Ignore;
        }
      }
    }
  ...
  if (inputType) {
    // QWidget::mapFromGlobal is very expensive on X11, so we cache the global
    ...
    if (receiverWidget) {
      ...
      point = receiverWidget->mapFrom(d->receiverWindow,
                                      point.toPoint() - d->receiverWindowPos);
  ...
  if (me || gsme) {
    ...
    else if (scrollerWasScrolling &&
             (scroller->state() == QtScroller::Dragging ||
              scroller->state() == QtScroller::Inactive))
  ...
  if (!inputType) {
    ...
    switch (event->type()) {
          ...
          int pressDelay =
              int(1000 *
                  scroller->scrollerProperties()
                      .scrollMetric(QtScrollerProperties::MousePressEventDelay)
                      .toReal());
          if (pressDelay > 0) {
            result |= ConsumeEventHint;
      ...
      case QEvent::GraphicsSceneMouseRelease:
      case QEvent::MouseButtonRelease:
        if (PressDelayHandler::instance()->released(
                event, scrollerWasDragging || scrollerWasScrolling,
                scrollerIsActive))
          result |= ConsumeEventHint;
      // fall through
      case QEvent::TouchEnd:
