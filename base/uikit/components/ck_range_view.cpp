...
class range_view::PrivateRangeWidget {
public:
  PrivateRangeWidget() {}
  ~PrivateRangeWidget() {}
...
range_view::range_view(widget *parent)
    : cherry_kit::widget(parent), o_range_widget(new PrivateRangeWidget) {
  this->set_widget_flag(cherry_kit::widget::kRenderBackground);
  this->set_widget_flag(cherry_kit::widget::kConvertToWindowType, false);
...
void range_view::setMaxValue(float maxValue) {
  o_range_widget->mMaxValue = maxValue;
}

float range_view::maxValue() const { return o_range_widget->mMaxValue; }

float range_view::currentValue() const {
  return o_range_widget->mProgressValue;
}

void range_view::dragMoveEvent(QGraphicsSceneDragDropEvent *event) {
  qDebug() << Q_FUNC_INFO << event->pos();
}

void range_view::mousePressEvent(QGraphicsSceneMouseEvent *event) {
  o_range_widget->mInitPos = mapToScene(event->pos());
  event->accept();
  QGraphicsItem::mousePressEvent(event);
}

void range_view::mouseReleaseEvent(QGraphicsSceneMouseEvent *event) {
  event->accept();
  // QGraphicsItem::mouseReleaseEvent(event);
}

void range_view::PrivateRangeWidget::calculateValue() {
  //
...
QString range_view::PrivateRangeWidget::convertAngleToTimeString(float angle) {
  QString time = "";
...
void range_view::mouseMoveEvent(QGraphicsSceneMouseEvent *event) {
  /*
...
void range_view::paint_view(QPainter *painter, const QRectF &rect) {
  QPen pen;
