...
class dial_view::PrivateDialWidget {
public:
...
dial_view::dial_view(widget *parent)
...
void dial_view::set_maximum_dial_value(float maxValue) {
  o_dial_widget->mMaxValue = maxValue;
}

float dial_view::maximum_dial_value() const {
  return o_dial_widget->mMaxValue;
}

float dial_view::current_dial_value() const {
  return o_dial_widget->mProgressValue;
}

void dial_view::reset() {
  o_dial_widget->mAngle = 0;
  o_dial_widget->mMaxAngle = 360;
  o_dial_widget->mProgressValue = 0;
  o_dial_widget->mStartAngle = 0;
...
void dial_view::on_dialed(std::function<void(int)> a_callback) {
  o_dial_widget->m_on_dial_callback_list.push_back(a_callback);
}

void dial_view::dragMoveEvent(QGraphicsSceneDragDropEvent *event) {
  qDebug() << Q_FUNC_INFO << event->pos();
}

void dial_view::mousePressEvent(QGraphicsSceneMouseEvent *event) {
...
void dial_view::mouseReleaseEvent(QGraphicsSceneMouseEvent *event) {
  event->accept();
  // QGraphicsItem::mouseReleaseEvent(event);
  o_dial_widget->m_is_pressed = 0;
}

void dial_view::PrivateDialWidget::calculateValue() {
  //
...
QString dial_view::PrivateDialWidget::convertAngleToTimeString(float angle) {
  QString time = "";
...
void dial_view::paint_view(QPainter *painter, const QRectF &rect) {
