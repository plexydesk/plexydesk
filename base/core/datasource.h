...
PlexyDesk data plugins are meant to abstract data source, such as data comming
from
flicker, a website or even your own local disk. The plugin would emit
dataReady() signal
when the data has been collected or if a data item is ready. once it's ready the
client
can read the abstracted data from readAll() function which returns a QVariantMap
of data.

When you are required to pass data to the plugin you should use pushData method
provided by
the plugin, this can be used for telling the plugin which youtube feeds you are
interested in,
or telling it which images to display from flicker. Code snippets below
illustrates how
...
When you need to provide a plugin of you own, simply inhert this and write a
class and wrapp
the class in a DataSource interface. Emit dataReady() signal when you data
source is ready to
be used, and provide the data from readAll method. look at the numours examples
shiped with
